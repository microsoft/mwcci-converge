...
import {
  TableProps, Table, tableCellClassName, tableClassName,
...
const ResponsiveTableContainer: React.FC<ResponsiveTableProps> = (props) => {
  const {
    columns: config, children, id, breakpoints: brkPoints,
  } = props;
   ...
   * the total size of the table ( based in th esum of the minWidth columns )
   * still fits in the current breakpoint
   */
  React.useLayoutEffect(() => {
    setBreakpoints(calculateBreakpoints(config, props.breakpoints || [1200, 600, 280]));
  }, [config, brkPoints]);
        ...
        {_.map(
          ...
          (breakpoint, key) => `
              @media (max-width: ${key}px) {
                ${breakpoint
            .map(
              (column) => `#${responsiveTableID} .${tableCellClassName}:nth-child(${column.childIndex}) { display: none }`,
            )
            .join("\n  ")}
              }`,
        ).join("\n")}
